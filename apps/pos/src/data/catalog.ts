// Mock catalog for the register (stands in for the CatalogModule API until we wire the backend).
...
export const TAX_RATES: Record<TaxGroupId, { name: string; rateBasisPoints: number }> = {
  standard: { name: 'Sales Tax (8.25%)', rateBasisPoints: 825 },
  food: { name: 'Food (0%)', rateBasisPoints: 0 },
  exempt: { name: 'Non-taxable', rateBasisPoints: 0 },
};

// Flat, saturated per-category colors — the "many different hues in one grid" is an X-Series signature.
...
export const CATALOG: CatalogItem[] = [
  ...
  // Cold drinks (bottled water & juice → food/0%; prepared iced tea → standard)
  ...
  // Retail merch (standard) + gift card (non-taxable)
